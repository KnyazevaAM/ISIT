...
# Берет определенное количесвто строк и матриц
def only_n_str(X, n):
    t = []
    for i in range(n):
        t.append(X[i])
...
# Находит индекс и само значение оценки фильма, который пользователь ещё не смотрел и имеет наивысший рейтинг
...
# находим нужный по уловию фильм
